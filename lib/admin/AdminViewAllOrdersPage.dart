// lib/admin/admin_view_all_orders_page.dart
import 'package:du_an_cuoi_ki/models/DonHangModel.dart';
import 'package:du_an_cuoi_ki/models/KhachHangModel.dart';
import 'package:du_an_cuoi_ki/models/ChiTietDonHangModel.dart';
import 'package:du_an_cuoi_ki/models/Product_Model.dart'; // Đảm bảo đường dẫn đúng
    ...
    try {
      ...
      for (var orderJson in ordersData) {
        ...
        final chiTietResponse = await _supabase
            .from('ChiTietDonHang')
            .select('id, id_donHang, id_sp, soLuong, gia')
                ...
                child: ExpansionTile( // Sử dụng ExpansionTile để hiển thị chi tiết
                  ...
                  children: order.chiTietDonHang.map((item) {
                    return ListTile(
                      ...
                      subtitle: Text("SL: ${item.soLuong} - Giá: ${currencyFormatter.format(item.gia)}"),
                    );
                  }).toList(),
                ),
              );

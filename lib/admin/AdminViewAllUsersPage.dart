import 'package:du_an_cuoi_ki/models/KhachHangModel.dart'; // Đảm bảo đường dẫn đúng
...
class _AdminViewAllUsersPageState extends State<AdminViewAllUsersPage> {
  ...
  Future<List<KhachHangModel>> _fetchUsers() async {
    try {
      // Sử dụng tên bảng và các cột từ thông tin bạn cung cấp
      final response = await _supabase
          .from('KhachHang') // Tên bảng là 'KhachHang'
          .select('id, tenKH, diaChi, soDienThoai'); // Các cột cần lấy
      ...
      //   SnackBar(content: Text('Lỗi khi tải dữ liệu khách hàng: ${e.toString()}')),
      // );
      ...
      body: FutureBuilder<List<KhachHangModel>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            // Có thể hiển thị lỗi chi tiết hơn cho việc debug

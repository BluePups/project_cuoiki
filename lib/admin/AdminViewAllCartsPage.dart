// lib/admin/admin_view_all_carts_page.dart
import 'package:du_an_cuoi_ki/models/GioHangItemModel.dart';
import 'package:du_an_cuoi_ki/models/KhachHangModel.dart';
import 'package:du_an_cuoi_ki/models/Product_Model.dart'; // Hoặc SanPhamModel
      ...
      for (var itemJson in cartData) {
        ...
        if (itemMap['id_sp'] != null) {
          final spResponse = await _supabase
              ...
              .maybeSingle();
          if (spResponse != null) {
            sanPham = Product.fromJson(spResponse as Map<String, dynamic>); // Hoặc SanPhamModel.fromJson
          }
        }
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<GioHangItemModel>>(
        ...
        builder: (context, snapshot) {
          ...
          // Nhóm các item theo người dùng để hiển thị đẹp hơn
          Map<String, List<GioHangItemModel>> itemsByUser = {};
          for (var item in items) {
            String userName = item.khachHang?.tenKH ?? item.idUser; // Dùng ID nếu không có tên
